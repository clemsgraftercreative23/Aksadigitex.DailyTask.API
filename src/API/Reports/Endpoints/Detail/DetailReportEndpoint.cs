...
public class DetailReportEndpoint : EndpointWithoutRequest<DetailReportResponse>
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        if (accountType == API.Auth.AuthAccountType.DirectorUser)
        {
            ...
            {
                var dirResp = dirReport.ToResponse();
                ...
                if (du != null)
                {
                    dirResp.UserFullName = du.FullName;
                    dirResp.UserEmail = du.Email;
                    dirResp.UserPosition = "Director";
                    dirResp.CompanyId = du.CompanyId;
                    dirResp.UserRoleName = "director";

                    if (du.CompanyId.HasValue)
                        dirResp.CompanyName = await _db.Companies.AsNoTracking()
                            .Where(c => c.Id == du.CompanyId.Value)
                            .Select(c => c.CompanyName)
                            .FirstOrDefaultAsync(ct);
                }

                await SendAsync(new DetailReportResponse { Item = dirResp }, cancellation: ct);
                return;
            }
        }

        // ---- Regular User: query daily_report (flow lama tidak berubah).
        // DirectorUser falls back here only if no director_report exists for this id.
